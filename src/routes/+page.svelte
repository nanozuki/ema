<script lang="ts">
  ...
  let { data } = $props();
  let thisYear = $derived(isCeremonyActive(data.ceremonies[0], data.now) ? data.ceremonies[0] : null);
  let formerYears = $derived(thisYear ? data.ceremonies.slice(1) : data.ceremonies);
</script>
